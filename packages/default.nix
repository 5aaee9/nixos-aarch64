  ...
  perSystem =
      ...
      buildRepartConfig =
        ...
        evalConfig {
          ...
          modules = [
            ...
            self.nixosModules.emergency-shell
            self.nixosModules.firstBoot
            self.nixosModules.apply-overlay
            self.nixosModules.cross
            { nixos-aarch64.emergencyShell.enable = lib.mkDefault true; }
          ]
          ++ config;
        };
      buildDefaultRepartConfig = kernelModule: buildRepartConfig system [ kernelModule ];
      buildDefaultRepartImage =
        kernelModule: (buildDefaultRepartConfig kernelModule).config.system.build.repartImage;
      systemdBootEsp =
        config: import ../modules/systemd-boot-esp-contents.nix { inherit config lib pkgs; };
    ...
    in
    {
      packages = rec {
        ...
        orangepi-3b-uboot = pkgsCross.callPackage ./orangepi-3b-uboot { inherit (inputs) rkbin; };
      ...
      checks =
        let
          assertOrThrow = condition: message: if condition then true else throw message;
          assertHasKeys =
            attrs: keys:
            assertOrThrow (
              (lib.sort lib.lessThan (builtins.attrNames attrs)) == (lib.sort lib.lessThan keys)
          ...
          layoutConfig = module: (buildDefaultRepartConfig module).config;
          makePlainCheck =
            name: module:
            let
          ...
          makeLayoutCheck =
            ...
            let
              ...
              result =
                ...
                && assertOrThrow (
                  cfg.system.build.repartImage.passthru.outputImage == "sd-image/${expectedName}.raw"
                ) "${name}: unexpected wrapper output"
                ...
                && assertOrThrow (
                  cfg.nixos-aarch64.repartImage.postBuildCommands != ""
                ) "${name}: missing default U-Boot post-build commands"
                && assertOrThrow (
                  root.device == "/dev/disk/by-label/NIXOS_ROOT" && root.fsType == "btrfs" && root.neededForBoot
                ) "${name}: invalid root filesystem"
                && assertOrThrow (
                  firmware.device == "/dev/disk/by-label/FIRMWARE"
                  && firmware.fsType == "vfat"
                  && builtins.elem "nofail" firmware.options
                  && !(builtins.elem "noauto" firmware.options)
                ...
                ) "${name}: invalid removable systemd-boot ESP source"
                && assertOrThrow (
                  espContents."/EFI/systemd/systemd-bootaa64.efi".source == esp.loaderSource
                ) "${name}: invalid managed systemd-boot ESP source"
                ...
                && assertOrThrow (
                  (bootContent esp.kernelFile).source == esp.kernelFile
                ) "${name}: invalid kernel ESP source"
                && assertOrThrow (
                  (bootContent esp.initrdFile).source == esp.initrdFile
                ) "${name}: invalid initrd ESP source"
                && assertOrThrow (
                  !esp.hasDeviceTree || (bootContent esp.deviceTreeFile).source == esp.deviceTreeFile
                ) "${name}: invalid device tree ESP source"
                && assertOrThrow (
                  !(espContents ? "/EFI/Linux/${cfg.system.boot.loader.ukiFile}")
                ) "${name}: unexpected build-time UKI ESP source"
                && assertOrThrow (
                  partitions."20-root".storePaths == [ cfg.system.build.toplevel ]
                ) "${name}: root storePaths missing toplevel"
                && assertOrThrow (
                  runtimeRoot.Type == "root-arm64"
                  && runtimeRoot.Format == "btrfs"
                  && runtimeRoot.Label == "NIXOS_ROOT"
                  && runtimeRoot.SizeMinBytes == "5G"
                  && runtimeRoot.PaddingMinBytes == "0"
                  && runtimeRoot.GrowFileSystem
                ) "${name}: invalid runtime root repart"
                ...
                && assertOrThrow (
                  !(partitions ? "30-store")
                  && !(partitions ? "30-store-a")
                  && !(partitions ? "30-store-b")
                  && !(partitions ? "40-var")
                ) "${name}: unexpected store or var partition";
            in
            pkgs.runCommand "${name}-repart-layout" { } ''
              ${lib.optionalString result "touch $out"}
            '';
          optionConfig =
            { verify, hydraBuildProduct }:
          ...
          verifyOn = optionConfig {
            verify = true;
            hydraBuildProduct = true;
          };
          verifyOff = optionConfig {
            verify = false;
          ...
          optionResult =
            assertOrThrow (
              verifyOn.system.build.repartImage.passthru.postBuildCommands != ""
            ) "postBuildCommands passthru missing"
            && assertOrThrow (
              builtins.match ".*img=\\$out/sd-image/option-check.raw.*echo \"\\$img\" > post-build-img-path.*" verifyOnCommand
              != null
            ) "postBuildCommands missing or not run after img assignment"
            && assertOrThrow (
              builtins.match ".*sgdisk --verify \"\\$img\".*" verifyOnCommand != null
            ) "verify=true missing sgdisk"
            && assertOrThrow (
              verifyOn.system.build.repartImage.passthru.nativeBuildInputPaths != [ ]
            ) "verify=true missing native inputs"
            && assertOrThrow (
              builtins.match ".*nix-support.*hydra-build-products.*" verifyOnCommand != null
            ) "hydra=true missing metadata write"
            ...
            && assertOrThrow (
              builtins.match ".*sgdisk --verify \"\\$img\".*" verifyOffCommand == null
            ) "verify=false still includes sgdisk"
            && assertOrThrow (
              verifyOff.system.build.repartImage.passthru.nativeBuildInputPaths == [ ]
            ) "verify=false still includes native inputs"
            && assertOrThrow (
              builtins.match ".*nix-support.*hydra-build-products.*" verifyOffCommand == null
            ) "hydra=false still writes metadata"
            && assertOrThrow (
              !verifyOff.system.build.repartImage.passthru.verify
            ) "verify=false passthru missing"
            && assertOrThrow (
              !verifyOff.system.build.repartImage.passthru.hydraBuildProduct
            ) "hydra=false passthru missing";
          customRootPartitions = customRootConfig.image.repart.partitions;
          customRootResult =
            assertHasKeys customRootPartitions [
              "00-uboot"
              "10-esp"
              "20-root"
            ]
            && assertOrThrow (
              customRootConfig.image.baseName == "custom-radxa-e20c"
            ) "custom root: unexpected image baseName"
            && assertOrThrow (
              customRootConfig.fileSystems."/".device == "/dev/disk/by-label/CUSTOM_ROOT"
            ) "custom root: root filesystem not overridden"
            && assertOrThrow (
              customRootConfig.systemd.repart.partitions."20-root".Label == "CUSTOM_ROOT"
            ) "custom root: runtime root label not overridden"
            && assertOrThrow (
              customRootPartitions."20-root".repartConfig.Label == "CUSTOM_ROOT"
            ) "custom root: image root label not overridden"
            && assertOrThrow (
              customRootPartitions."20-root".repartConfig.SizeMinBytes == "6G"
            ) "custom root: image root size not overridden"
            && assertOrThrow (
              customRootPartitions."10-esp".repartConfig.Label == "FIRMWARE"
            ) "custom root: default ESP missing"
            && assertOrThrow (
              customRootPartitions."00-uboot".repartConfig.SizeMinBytes == "15M"
            ) "custom root: default U-Boot partition missing";
        ...
        in
        {
          ...
          fastrhino-r68s-repart-layout =
            makeLayoutCheck "fastrhino-r68s" "nixos-fastrhino-r68s-repart"
              fastrhinoRepartConfig;
